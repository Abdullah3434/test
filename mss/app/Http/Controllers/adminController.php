...
$sale_type = "sale";
        DB::insert("insert into sale(bill_date,sale,customer_name,account_type,sale_type,company_name,vehicle_no,total_amount,created_at) values (?,?,?,?,?,?,?,?,?)",array($request->input('date'),$sale,$request->input('customer_name'),$request->input('account_type'),$sale_type,$request->input('company_name'),$request->input('vehicle_no'),$total_amount,$request->input('date')));
...
// return $customer;
          $result = DB::select("select* from sale where customer_name = '$id' and sale = 'invoice'");
          // return $result;
...
if($request->input('account_type') == "credit")
{
  ...
  if($request->input('account_type') == "cash")
  {
      ...
      DB::insert("insert into account(account_type,account_name,description,balance,date) values (?,?,?,?,?)",array('Cash On Hand','Cash On Hand','Cash On Hand',$total_amount,NOW()));
          ...
          //  if($request->input('account_type'))
          //   { 
          //      DB::insert("insert into account_receivable (customer_name,date,amount_received,receiving_method,receiving_account,acount_type_id) values (?,?,?,?,?,?)",array($request->input('customer_name'),$request->input('date'),0,$receiving_method,$total_amount,$request->input('account_type')));  
          ...
          //       $account = DB::select("select* from account where pk_id =".$account_type_id);
          //       $purchase_account = DB::select("select* from account where account_type = 'Purchase Manegment'");
          //       $recievable_account = DB::select("select* from account where account_type = 'Account Receivable'");
                ...
                //     $c_balance = $account[0]->balance + $total_amount; 
                //     $account_recievable_balance = $recievable_account[0]->balance + $total_amount; 
                //     if(!empty($purchase_account[0]->balance))
                //     {
                //      $puchase_amount = $purchase_account[0]->balance-$total_amount;
                //     }else{
                //      $puchase_amount = 0;
                ...
                //     $result = DB::table('account')->where('pk_id',$account[0]->pk_id)->update(['balance' =>$c_balance]);
                //     $result = DB::table('account')->where('account_type','Purchase Manegment')->update(['balance' =>$puchase_amount]);
                //      $result = DB::table('account')->where('account_type','Account Receivable')->update(['balance' =>$account_recievable_balance]);
...
// dd($request->all());
             ...
             if($request->input('account_type'))
            {
              ...
              DB::insert("insert into account_payable (supplier_name,date,amount_payed,paying_method,paying_account) values (?,?,?,?,?)",array($request->input('supplier_name'),$request->input('date'),0,"Bank",$total_amount)); 

              DB::enableQueryLog();
               $account_type_id = $request->input('account_type');
                $account = DB::select("select* from account where pk_id =".$account_type_id);
                  $sale_account = DB::select("select* from account where account_type = 'Sale Manegment'");
                  $purchase_account = DB::select("select* from account where account_type = 'Account Payable'");
                             $query = DB::getQueryLog();
                // print_r($query);
                // dd($account, $account_type_id);
                ...
                //     $c_balance = $account[0]->balance - $total_amount; 
                //     $balance_sum = $purchase_account[0]->balance + $total_amount; 
                //     $sale_balance = $sale_account[0]->balance - $total_amount; 
                //     $resultt = DB::table('account')->where('pk_id',$account[0]->pk_id)->update(['balance' =>$c_balance]); 
                //     $resultt = DB::table('account')->where('account_type','Sale Manegment')->update(['balance' =>$sale_balance]);
                //     $resultt = DB::table('account')->where('account_type','Account Payable')->update(['balance' =>$balance_sum]);
                //     $purchase_account = DB::select("select* from account where account_type = 'Purchase Manegment'");
                //     $sale_balance = $purchase_account[0]->balance + $total_amount; 
                //     $resultt = DB::table('account')->where('pk_id',$purchase_account[0]->pk_id)->update(['balance' =>$sale_balance]);
                ...
                //     DB::insert("insert into account(account_type,account_name,description,balance,date) values (?,?,?,?,?)",array('Account Purchase','Account Purchase','Account Purchase',$total_amount,NOW()));
             ...
             for($i = 0;  $i < $wordCount ; $i++)
             {
                DB::insert("insert into detail_tax_purchase (purchase_id,sku,item_name,quantity,rate,tax,tax_amount,amount) values (?,?,?,?,?,?,?,?)",array($result[0]->pk_id,$sku[$i],$item_name[$i],$quantity[$i],$rate[$i],$tax[$i],$tax_amount[$i],$amount[$i]));
             }
...
$result = "Select* from sale where ";
            ...
            if($request->input('date_from'))
          {
           ...
           $result .= "created_at BETWEEN '$date_from' AND '$date_to' ";
            ...
            return view('admin.sale_by_customer_list_view',compact('result1','result','total_amount'));
...
$detail_purchase_sum = DB::table('detail_purchase')->where('purchase_type', 'purchase')->sum('amount');


$detail_purchase_return =  DB::select("select* from detail_purchase where purchase_type='return'  ");
// return $detail_purchase_return;
$detail_purchase_sum_return = DB::table('detail_purchase')->where('purchase_type', 'return')->sum('amount');
